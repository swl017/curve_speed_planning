...
curve_speed_planning::curve_speed_planning()
{
    ...
    ros::NodeHandle nh;
    path_sub_      = nh.subscribe("path", 1, &curve_speed_planning::pathSubCallback, this); // DK CHECK
    ego_state_sub_ = nh.subscribe("pose_estimate", 1, &curve_speed_planning::egoStateSubCallback, this); // DK CHECK

    speed_command_pub_ = nh.advertise<std_msgs::Float64>("speed_command", 1); // DK CHECK
    speed_plan_pub_    = nh.advertise<std_msgs::Float64MultiArray>("speed_command", 1); // DK CHECK
}
...
{
    if(msg->poses.size() > 5){
        speed_command_ = getSpeedCommand(current_speed_, msg);
...
double curve_speed_planning::getSpeedCommand(double current_speed, const nav_msgs::PathConstPtr &path)
{
...
{
    double dx = pose_from.pose.position.x - pose_to.pose.position.x;
    double dy = pose_from.pose.position.y - pose_to.pose.position.y;
